...
class TestBenchmark(unittest.TestCase):
    def test_benchmarking_1_dataset(self):
        import imbalanced_databases as imbd
        from sklearn.neighbors import KNeighborsClassifier
        from sklearn.tree import DecisionTreeClassifier
        data= imbd.load_glass2()
        X= data['data']
        y= data['target']
...
class TestModelSelection(unittest.TestCase):
    def model_selection(self):
        ...
        self.assertTrue((not samp_obj is None) and (not cl_obj is None))
...
class TestMLPWrapper(unittest.TestCase):
    def test_mlp_wrapper(self):
        ...
        self.assertTrue(not classifier is None)
